...
> I’m an applied ML research scientist focused on sequential and multimodal data. Most recently I’ve been leading the technical direction on multimodal time-series models — designing dual visual encodings into an LLM, owning multi-stage training curricula, and building a strict eval harness so we only keep changes that actually move the metrics. Before that, my work was generative and representation learning for time series — including image-based transforms and irregular sampling. I have a PhD from the Technion and publish in NeurIPS, ICML, and ICLR. I’m a US permanent resident, I’m based in Seattle, and I’m looking for a full-time IC role where I can lead multimodality research hands-on — architecture, experiments, and the decisions that gate what ships. Health AIML is a strong fit because it’s foundational multimodal models over health and fitness signals at Apple scale — and I’m motivated by doing that carefully, with eval and safety first.
