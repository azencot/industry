# Org graph — Sapiro / Shirley / Giryes / Kimmel (public only)

**When:** 2026-08-21 (same day as HM + Tyler advance)  
**Use:** people map for optional outreach. **Not** a claim about reporting lines. Apple rarely labels Health AIML vs Health AI cleanly.  
**Loop rule:** Tyler owns process. **Do not email Shirley.** Do not have anyone lobby her during the tech screen.

---

## Shirley ↔ Guillermo Sapiro

**Sapiro (public):** Distinguished Engineer at Apple; LinkedIn: **Health AI & Personalized AI for Science**. Princeton Augustine Family Professor (started ~Jan 2025 after Duke). Princeton / Apple bios: he **leads a team on Health AI**. Dual appointment, not a full-time IC on Shirley’s req.

**Shirley (public):** Senior ML Manager / Principal Engineer, **Health & Fitness**. HM for this Seattle Health AIML RS req.

**Documented overlap — collaboration, not a proven report line**

| Evidence | What it shows |
|----------|----------------|
| [npj Digital Medicine 2023](https://www.nature.com/articles/s41746-023-00926-4) — Nazaret, Tonekaboni, Darnell, **Ren**, **Sapiro**, Miller | Personalized HR response on **Apple Heart and Movement Study** wearables. All authors Apple-employed for the work. Author contributions: **Darnell, Ren, and Sapiro** did methods/analysis and **provided supervision**; Miller corresponding. |
| NeurIPS 2022 Apple list | Same HR workshop paper with the same author set. |
| ICLR 2025 [Do LLMs “know” internally…](https://proceedings.iclr.cc/paper_files/paper/2025/file/ca6980a3dba7fb3e4e66925656dba68b-Paper-Conference.pdf) (Heo et al.; **Shirley Ren**, Miller, Narain, …) | Internship at **Apple AIML**. Acknowledgements: thanks **Guillermo Sapiro** for “unwavering support and guidance.” Sponsor/senior-scientist signal, not a byline. |

**Honest read:** they work in the **same Health AI / Health & Fitness cluster**. Sapiro is **more senior** (Distinguished Engineer; “leads Health AI”). Shirley is the **line manager** for this req. Public sources do **not** prove he is her skip-level manager vs a scientific Distinguished Engineer who co-supervises papers with her group. Do not tell interviewers “Sapiro is Shirley’s boss.”

Shared coauthor **Gregory Darnell** sits on both the HR paper and **RelCon** (with Shirley) — one more edge in that cluster.

---

## Ron Kimmel → Sapiro (and to you)

**Ron → Guillermo:** real, old, high-status science — geodesic active contours / minimal surfaces (Caselles, Kimmel, Sapiro); Helmholtz Test-of-Time (ICCV) for the 1995 paper. Not an Apple reporting edge.

**Ron → you (locked 2026-08-21):** not a weak ask. He was on your **PhD committee**, followed the work after, and was a **promotion letter writer**. He thinks highly of you. That is a legitimate academic referral, not a cold LinkedIn intro.

**What happened (2026-08-22):** Ron mailed `guillermo.sapiro@gmail.com` (“dear friend and colleague”; contact anytime). **Omri sent the follow-up the same day** — sent text in [`2026-08-22_kimmel-sapiro-outreach.md`](2026-08-22_kimmel-sapiro-outreach.md). Guillermo may or may not sit on this req.

**What to do now:** wait. Do not chase before Tue 8/25. If he offers a call: science + group texture, not “please ping Shirley.”

**Hard rules:**
1. You / Ron → **Guillermo only** — **not Shirley**, not Feng, not Tyler.
2. The note is “Ron introduced us; I am in process with Health AIML” — **not** “please hire me onto Shirley’s req.”
3. Do **not** name-drop Ron or Guillermo on Tuesday with Feng.

---

## Raja Giryes at Apple

**Not Health AIML on public evidence.** Wrong org for this req unless you have private info that contradicts the below.

| Signal | Detail |
|--------|--------|
| Role | TAU EE professor; LinkedIn: **Research Scientist at Apple** since **Jul 2025**, **on sabbatical**. |
| Public post | Sabbatical at **Apple AI Research**, collaborating with **Vladlen Koltun**. |
| Paper | [ICASSP 2026](https://machinelearning.apple.com/research/coarse-grained) *Principled Coarse-Grained Acceptance for Speculative Decoding in Speech* — Yanuka, Dixon, Finkelshtein, Rotman, **Giryes**. Apple ML Research tags: speech/NLP, methods. Giryes affiliation **† Tel Aviv University**; coauthors look like **Apple UK** speech/TTS, not Health & Fitness. |
| Sapiro link | Historical: Giryes **postdoc with Sapiro at Duke (2014–2015)**. That is lineage, not “Raja sits on Shirley’s team.” |

Do **not** ask Raja to ping Shirley for this loop. Optional only if you already know him: coffee on **Apple AI Research vs Health**, not a referral into req 200670570.

---

## Outreach (locked)

| Do | Don’t |
|----|--------|
| **Wait** — follow-up already sent ([`2026-08-22_kimmel-sapiro-outreach.md`](2026-08-22_kimmel-sapiro-outreach.md)) | Email Shirley or Feng; chase Guillermo before Tue |
| Keep it scientific; you are in process, not asking him to intervene | “Please tell Shirley to hire me” |
| One note; do not chase before Tue 8/25 | Name-drop Ron or Sapiro on the tech screen |
| Keep Raja as AI-Research/speech context | Treat Raja as Health AIML access |
